...
void show_file_selection_dialog(t_app_data *papp_data, int type, bool& bcancel)
{
  ...
  bcancel = false;
  papp_data->pfnfc->type(type);
  switch (papp_data->pfnfc->show())
    {
    case -1:
      printf("ERROR: %s\n" , papp_data->pfnfc->errmsg());
      break; // ERROR
    case 1:
      //printf("CANCEL\n" );
      bcancel = true;
      break; // CANCEL
    default:
      strncpy(str, papp_data->pfnfc->filename(), cstrsz);
      for (i = strlen(str) - 1; i > 0; i--)
	{
      ...
      papp_data->music_path = string(str);
      papp_data->filename = string(papp_data->pfnfc->filename());
#ifdef _DEBUG
...
void quicksave_callback(Fl_Widget *pwi, void *pdata)
{
  ...
  if (papp_data->filename.length() == 0)
    {
      show_file_selection_dialog(papp_data, Fl_Native_File_Chooser::BROWSE_FILE, bcancel);
    }
  if (!bcancel)
    {
...
void save_callback(Fl_Widget *pwi, void *pdata)
{
  ...
  show_file_selection_dialog(papp_data, Fl_Native_File_Chooser::BROWSE_SAVE_FILE, bcancel);
  if (!bcancel)
    {
      message = coder.create_file_os_message(path, papp_data->filename, efilesave);
      if (coder.build_wire_message(msg, cmsgsize, &size, message))
