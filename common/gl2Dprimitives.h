...
{
 public:
  CGL2Dprimitives(SDL_Window *psdl_window, SDL_GLContext GLContext, int screen_w, int screen_h);
  ~CGL2Dprimitives();
  ...
  void set_dft_size(int w, int h);
  void update_spectrum_texture_chunk(int xstart, int xstop, int width, int height, void *pmap);
  void draw_spectrum_texture(Cgfxarea *pw, int cut);
  void moulagaufre(std::string texturename, t_fcoord pos, t_fcoord tdim, float angle);
  void moulagaufre_box(t_fcoord pos, t_fcoord tdim, float angle, int color, float radius);
 ...
 private:
  ...
  void create_sp_texture(t_coord dim);
